...
export class InvoiceService {
  ...
  private async buildAndUpload(data: InvoiceData, type: 'invoice' | 'receipt'): Promise<UploadedDocument> {
    ...
    const verificationUrl = `${frontendUrl}/verify-bill/${data.bookingId}`;
    
    // Generate QR code buffer (PNG)
    const qrBuffer = await QRCode.toBuffer(verificationUrl, { margin: 1, width: 150 });
    
    await this.writePdf(data, type, tmpFile, qrBuffer);
  ...
  renderPdfDocument(doc: PDFKit.PDFDocument, data: InvoiceData, type: 'invoice' | 'receipt', qrBuffer: Buffer): void {
    ...
    let y = ML;
    y = this.header(doc, data, docNumber, docTitle, y, qrBuffer);
  ...
  private writePdf(data: InvoiceData, type: 'invoice' | 'receipt', outputPath: string, qrBuffer: Buffer): Promise<void> {
    return new Promise((resolve, reject) => {
      const doc = new PDFDocument({ margin: 0, size: 'A4', autoFirstPage: true });
      const stream = fs.createWriteStream(outputPath);
      doc.pipe(stream);
      this.renderPdfDocument(doc, data, type, qrBuffer);
      stream.on('finish', resolve);
      stream.on('error', reject);
    });
  }

  private header(doc: PDFKit.PDFDocument, data: InvoiceData, docNumber: string, docTitle: string, y: number, qrBuffer: Buffer): number {
    ...
    // Draw QR code top right (48x48)
    const qrSize = 48;
    const qrX = PW - MR - qrSize;
    doc.image(qrBuffer, qrX, y, { width: qrSize, height: qrSize });

    // Title right (shifted left by qrSize + 10)
    doc.fillColor(C.text).fontSize(22).font('Helvetica-Bold')
      .text(docTitle, ML, y, { width: CW - qrSize - 10, align: 'right' });

    // Invoice meta
    const metaY = y + 26;
    doc.fillColor(C.muted).fontSize(8).font('Helvetica')
      .text(`INVOICE NO: ${docNumber}`, ML, metaY, { width: CW - qrSize - 10, align: 'right' });
    doc.text(
      `DATE: ${data.paidAt.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}`,
      ML, metaY + 11, { width: CW - qrSize - 10, align: 'right' }
    );

    // PAID badge
    const badgeY = metaY + 24;
    doc.roundedRect(PW - MR - qrSize - 10 - 46, badgeY, 46, 15, 3).fill(C.green);
    doc.fillColor(C.white).fontSize(8).font('Helvetica-Bold')
      .text('● PAID', PW - MR - qrSize - 10 - 44, badgeY + 4, { width: 42, align: 'center' });
